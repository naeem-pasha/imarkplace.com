...
class Customer extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function odooCustomerCreate($customerArray, $mageCustomerId, $mageAddressId, $storeId = 0)
    {
        ...
        $resp = $helper->callOdooMethod('res.partner', 'create', [$customerArray]);
        if ($resp && $resp[0]) {
            $odooId = $resp[1];
            if ($odooId && $mageCustomerId && $mageAddressId) {
                ...
                $resp = $helper->callOdooMethod('connector.partner.mapping', 'create', [$mapArray]);
            }
        } else {
            $respMessage = $resp[1];
    ...
    public function odooCustomerUpdate($customerId, $addressId, $addressArray, $odooCustomerId)
    {
        ...
        $resp = $helper->callOdooMethod('res.partner', 'write', [$odooCustomerId, $addressArray]);
        if ($resp && $resp[0]) {
            $response = true;
        } else {
            $respMessage = $resp[1];
