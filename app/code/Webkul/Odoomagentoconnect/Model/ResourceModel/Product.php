...
class Product extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function getProductCategoryArray($categoryIds)
    {
        ...
        foreach ($categoryIds as $catId) {
            ...
            foreach ($mapcollection as $map) {
                ...
                array_push($odooCategories, (int)$odooId);
            }
        }
        if (!$odooCategories) {
    ...
    public function getProductArray($productId)
    {
        ...
        $productArray = [
                ...
                'standard_price'=>(float)$product->getCost(),
        ];
    ...
    public function updateNormalProduct($mappingObj)
    {
        ...
        if ($mappingObj) {
            ...
            $resp = $helper->callOdooMethod('product.product', 'write', [$odooId, $productArray]);
            if ($resp && $resp[0]) {
                $response['odoo_id'] = $odooId;
    ...
    public function createInventoryAtOdoo($mageProdId, $odooProId)
    {
        ...
        $resp = $helper->callOdooMethod('connector.snippet', 'update_quantity', [$inventoryArray], true);
        if ($resp && $resp[0]) {
            return true;
        } else {
            $respMessage = $resp[1];
