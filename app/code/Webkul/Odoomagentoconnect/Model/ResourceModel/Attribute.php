...
class Attribute extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function getMageAttributeArray()
    {
        ...
        foreach ($collection as $value) {
            if ($value['frontend_input']=='select' && $value->getAttributeCode()!='google_merchant_center_catego') {
    ...
    public function mapAttribute($data)
    {
        ...
        $resp = $helper->callOdooMethod('connector.attribute.mapping', 'create', [$attrMappingArray]);
        if ($resp && $resp[0]) {
            return true;
        }
        return false;
