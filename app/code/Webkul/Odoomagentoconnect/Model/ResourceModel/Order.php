...
class Order extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function createOdooOrderLine($thisOrder, $odooId, $thisQuote=false)
    {
        ...
        foreach ($items as $item) {
            ...
            $product = $helper->createModel(\Magento\Catalog\Model\Product::class)->load($productId);
            if ($priceIncludesTax) {
    ...
    public function createOdooOrderShippingLine($thisOrder, $odooId)
    {
        ...
        $shippingIncludesTax = $helper->getStoreConfig('tax/calculation/shipping_includes_tax');
        $priceIncludesTax = $helper->getStoreConfig('tax/calculation/price_includes_tax');
        if ($priceIncludesTax) { // Checking product inclusive config because MOB sync tax setting based on this.
    ...
    public function checkAddresses($thisOrder)
    {
        ...
        if ($thisOrder->getShippingAddressId() && $thisOrder->getBillingAddressId()) {
            ...
            switch ($flag) {
                case $s['street'] != $b['street']:
                    $flag = true;
                    break;
                case $s['postcode'] != $b['postcode']:
                    $flag = true;
                    break;
                case $s['city'] != $b['city']:
                    $flag = true;
                    break;
                case $s['region'] != $b['region']:
                    $flag = true;
                    break;
                case $s['country_id'] != $b['country_id']:
                    $flag = true;
                    break;
                case $s['firstname'] != $b['firstname']:
                    $flag = true;
                    break;
                default:
                    $flag = false;
