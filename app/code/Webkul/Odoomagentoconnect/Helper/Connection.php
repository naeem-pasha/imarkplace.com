...
class Connection extends \Magento\Search\Helper\Data
{
    ...
    public function checkOdooActiveConfig()
    {
        ...
        if ($userId > 0) {
            ...
            $fields = ['magento_url'=>new xmlrpcval($mageUrl, "string"),'ecomm_type'=>new xmlrpcval('magento2', "string"),];
    ...
    public function checkConfigurationChange()
    {
        $flag = true;
        switch ($flag) {
            case self::$odooUrl != $this->getSession()->getOdooUrl():
                $flag = false;
                break;
            case self::$odooPort != $this->getSession()->getOdooPort():
                $flag = false;
                break;
            case self::$odooDb != $this->getSession()->getOdooDb():
                $flag = false;
                break;
            case self::$odooUser != $this->getSession()->getOdooUser():
                $flag = false;
                break;
            case self::$odooPwd != $this->getSession()->getOdooPwd():
                $flag = false;
                break;
        }
      
        if (!$flag) $this->setOdooConfigurationInSession();
        return $flag;
    }
    ...
    public function getOdooContext()
    {
        ...
        $context = [
                "lang"=>$defaultLang,
                "magento2"=>"magento2",
                "instance_id"=>(int)$defaultInstance,
                ];
        return $context;
    }
    ...
    public function callOdooMethod($class, $method, $parameters, $needContext = true)
    {
        ...
        if ($userId > 0) {
            ...
            if ($needContext) {
                ...
                if (is_array($needContext) == "array") {
                    $context = array_merge($context, $needContext);
                }
