<?php
/**
 * Webkul Odoomarketplaceconnect ApproveProductObserver
 *
 * @category  Webkul
...
/**
 * Webkul Odoomarketplaceconnect ApproveProductObserver Observer.
 */
class ApproveProductObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($productObj) {
            ...
            foreach ($collection as $value) {
                ...
                $response = $helper->callOdooMethod($model, "write", [(int)$odooId, ['status'=> 'approved']]);
                if ($response && $response[0]) {
                    $message = "The Seller Product have been successfully approved at Odoo.";
                    $this->_messageManager->addSuccess(__($message));
                } else {
                    $message = 'The Seller Product have not been approved at Odoo for more details check Synchronization Logs !!';
