 ...
 */
class SaveAnalyticData extends \Magento\Customer\Controller\AbstractAccount
{
    /**
            ...
            try {
                ...
                if ($salePerPartnerColl->getSize() == 1) {
                    foreach ($salePerPartnerColl as $verifyrow) {
                        $verifyrow->setAnalyticId($fields['analytic_id']);
                        $verifyrow->save();
                    }
                } else {
                    $collectioninsert = $this->mpSalesPartner->create();
                    $collectioninsert->setSellerId($sellerId);
                    $collectioninsert->setAnalyticId($fields['analytic_id']);
                    $collectioninsert->save();
                }
                $this->messageManager->addSuccess(
                    __('Google Analytic information was successfully saved')
                );
            } catch (\Exception $e) {
                $this->helper->logDataInLogger(
                    "Controller_Account_SaveAnalyticData execute : ".$e->getMessage()
                );
