...
class Customer extends \Magento\Rule\Model\Condition\AbstractCondition
{
    ...
    protected function setReferredAttributes($customer)
    {
        ...
        $customer->setData(self::OPTION_IS_REFERRAL, (int) $this->isReferee($customer));
        $customer->setData(self::OPTION_IS_REFEREE, (int) $this->isReferral($customer));
    ...
    protected function getSumOfSpentPointsByCustomer($customer)
    {
        ...
        $transactions->getSelect()
            ...
            ->columns(new \Zend_Db_Expr('SUM(amount_used) as spent'));
