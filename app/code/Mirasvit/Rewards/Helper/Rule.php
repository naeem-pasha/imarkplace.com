...
class Rule extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @param string             $name
     * @param string             $current
     * @param string|int|\Magento\Framework\Data\Form\Element\AbstractElement $attribute
     * @param string             $tags
     * @return string
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    ...
    public function replaceCurrencyVariable($message)
    {
        ...
        if (preg_match_all('/##\d{1,}(\.\d{1,})?/', $message, $match) && count($match[0])) {
            foreach ($match[0] as $variable) {
                ...
                $message = str_replace($variable, $this->pricingHelper->currency((float)$price, true, false), $message);
            }
