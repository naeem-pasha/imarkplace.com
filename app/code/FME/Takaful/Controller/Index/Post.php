...
class Post extends \FME\Takaful\Controller\Index
{
        
        const CONFIG_CAPTCHA_ENABLE = 'takaful/google_options/captchastatus';
        const CONFIG_CAPTCHA_PRIVATE_KEY = 'takaful/google_options/googleprivatekey';
    ...
    private function _processFileUpload()
    {
        ...
        try {
            ...
            if (isset($result) && !empty($result['name'])) {
                ...
                $quickrfq_dir = $media_dir.'/takaful/';
                                
                                
                        $Uploader->save($quickrfq_dir);
                        return 'takaful/'.$Uploader->getUploadedFileName();
            }
        } catch (\Exception $e) {
                    $this->messageManager->addError(
                        __($e->getMessage())
