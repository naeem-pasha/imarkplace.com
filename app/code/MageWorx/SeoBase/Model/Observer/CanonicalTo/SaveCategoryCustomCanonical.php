...
class SaveCategoryCustomCanonical implements ObserverInterface
{
    /**
    ...
    /**
     * SaveCategoryCustomCanonical constructor.
     *
    ...
    {
        $entity = $observer->getEvent()->getCategory();
        ...
        if ($canonicalUrlType == CanonicalUrlType::TYPE_DEFAULT && $customCanonicalId) {
            ...
            && !empty($entityData['mageworx_seobase_category_canonical'])
        ) {
    ...
    {
        $canonicalToData = $entityData['mageworx_seobase_category_canonical'];

        if (empty($entityData['store_id'])) {
        ...
        $data = [
            CustomCanonicalInterface::SOURCE_ENTITY_TYPE => Rewrite::ENTITY_TYPE_CATEGORY,
