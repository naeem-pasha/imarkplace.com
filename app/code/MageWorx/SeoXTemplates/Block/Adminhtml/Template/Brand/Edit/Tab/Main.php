...
use MageWorx\SeoXTemplates\Model\Template\Brand\Source\AssignType as AssignTypeOptions;
...
use MageWorx\SeoXTemplates\Helper\Comment\Brand as HelperComment;
...
class Main extends GenericForm implements TabInterface
{
    ...
    protected function getLegendText()
    {
        ...
        if ($storeId == 'default' || $this->getBrandTemplate()->getUseForDefaultValue()) {
    ...
    protected function getIsSingleStoreMode()
    {
        if (is_null($this->getRequest()->getParam('is_new'))) {
            return $this->getBrandTemplate()->getIsSingleStoreMode();
        }
    ...
    protected function getAssignTypes()
    {
        ...
        if ($this->getBrandTemplate()->getDuplicateTemplateAssignedForAll()) {
            foreach ($options as $key => $option) {
                if ($option['value'] == \MageWorx\SeoXTemplates\Model\Template\Brand::ASSIGN_ALL_ITEMS) {
                    unset($options[$key]);
                }
            }
        }
        ...
        "<script>
            ...
            $('#template_assign_type1').on('change', function() {
                ...
                if ($('#template_assign_type1:checked')) {
                    $('li[aria-labelledby=template_brand_tabs_brands]').hide();
                }
            });
            ...
            $('#template_assign_type2').on('change', function() {
                ...
                if ($('#template_assign_type2:checked')) {
                    $('li[aria-labelledby=template_brand_tabs_brands]').show();
                }
            });
        });
        </script>";
