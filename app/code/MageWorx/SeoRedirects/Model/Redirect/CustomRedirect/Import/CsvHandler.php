...
class CsvHandler extends CsvHandlerAbstract
{
    ...
    protected function validateByDataFormat(array $data)
    {
        ...
        foreach ($data as $rowIndex => $dataRow) {
            ...
            if (!empty($dataRow[6])) {
                $this->validateDateRow($dataRow[6], $rowIndex);
            }

            if (!empty($dataRow[7])) {
                $this->validateDateRow($dataRow[7], $rowIndex);
            }
    ...
    protected function splitOnStoresAndCheckInternalDuplicates(array $data)
    {
        ...
        foreach ($data as $dataRow) {
            ...
            $dataRow[6] = empty($dataRow[6]) ? '' : $dataRow[6];
            $dataRow[7] = empty($dataRow[7]) ? '' : $dataRow[7];
            ...
            $dataRow = array_combine(
                [
                    ...
                    CustomRedirect::START_AT,
                    CustomRedirect::FINISH_AT
                ],
