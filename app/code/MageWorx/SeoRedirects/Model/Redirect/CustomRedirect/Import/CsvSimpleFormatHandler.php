...
class CsvSimpleFormatHandler extends CsvHandlerAbstract
{
    ...
    protected function validateByDataFormat(array $data)
    {
        ...
        foreach ($data as $rowIndex => $dataRow) {
            ...
            if (!empty($dataRow[4])) {
                $this->validateDateRow($dataRow[4], $rowIndex);
            }

            if (!empty($dataRow[5])) {
                $this->validateDateRow($dataRow[5], $rowIndex);
            }
    ...
    protected function splitOnStoresAndCheckInternalDuplicates(array $data)
    {
        ...
        foreach ($data as $dataRow) {
            ...
            $dataRow[4] = empty($dataRow[4]) ? '' : $dataRow[4];
            $dataRow[5] = empty($dataRow[5]) ? '' : $dataRow[5];

            $dataRow = array_combine(
                ['request_entity_identifier', 'target_entity_identifier', 'redirect_code', 'store_id', 'start_at', 'finish_at'],
        ...
        foreach ($storeSplitData as $storeId => $data) {
            foreach ($data as $dataRow) {
                ...
                $redirect
                    ...
                    ->setStartAt($dataRow[CustomRedirect::START_AT])
                    ->setFinishAt($dataRow[CustomRedirect::FINISH_AT])
                    ->setIsImported(true);
