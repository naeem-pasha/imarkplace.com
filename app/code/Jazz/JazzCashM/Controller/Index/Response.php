...
namespace Jazz\JazzCashM\Controller\Index;
...
class Response extends \Magento\Framework\App\Action\Action {
	...
	public function execute() {
		...
		if ($this->getRequest ()->isPost ()) {
			...
			$_MerchantID = $this->scopeConfig->getValue ( 'payment/jazz_cashm/merchant_ID', ScopeInterface::SCOPE_STORE );
			$_Password = $this->scopeConfig->getValue ( 'payment/jazz_cashm/password', ScopeInterface::SCOPE_STORE );
			$_IntegritySalt = $this->scopeConfig->getValue ( 'payment/jazz_cashm/integrity_salt', ScopeInterface::SCOPE_STORE );
			$_ValidateHash = $this->scopeConfig->getValue ( 'payment/jazz_cashm/hash_check', ScopeInterface::SCOPE_STORE );
			...
			if ($_ValidateHash) {
				...
				if (! $this->isNullOrEmptyString ( $_SecureHash )) {
					...
					if (strtolower ( $CalSecureHash ) == strtolower ( $_SecureHash )) {
						...
						$isResponseOk = false;
						$errorMsg = 'JazzCashM: Secure Hash mismatched!';
						$comment .= "Secure Hash mismatched.";
					}
				} else {
					$isResponseOk = false;
					$errorMsg = 'JazzCashM: Secure Hash is empty!';
			...
			if ($isResponseOk) {
				...
				if (in_array ( $_ResponseCode, $payment_success_codes )) {
					...
					$successFlag = false;
					// $order->setStatus('JazzCashM_failed');
					// $order->setExtOrderId($_TxnRefNo);
					
					if ($_TxnType == "MWALLET") {
						
						$order->setStatus ( 'mw_failed' );
						$order->setState ( 'mw_failed' );
						$order->setExtOrderId ( $_TxnRefNo );
					} else if ($_TxnType == "OTC") {
						
						$order->setStatus ( 'otc_failure' );
						$order->setState ( 'otc_failure' );
						$order->setExtOrderId ( $_TxnRefNo );
					} else if ($_TxnType == "MIGS") {
						
						$order->setStatus ( 'migs_failure' );
						$order->setState ( 'migs_failure' );
						$order->setExtOrderId ( $_TxnRefNo );
					}
					$this->cancelAction ();
